...
def conv2d_same(inputs, num_outputs, kernel_size, stride, scope = None):
    if stride == 1:
        ...
        pad_beg = pad_total // 2      #整除
        pad_end = pad_total - pad_beg
        inputs = tf.pad(inputs, [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]])  #补零输入变量
        ...
        with tf.variable_scope(block.scope, 'block', [net]) as sc:
            for i, unit in enumerate(block.args):
                with tf.variable_scope('unit_%d' %(i + 1), values = [net]):
                    #unit为(256, 64, 1)为类似 unit_depth = 256 unit_depth_bottleneck = 64 unit_stride = 1
                    unit_depth, unit_depth_bottleneck, unit_stride = unit
                    #调用bottleneck生成残差网络unit
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections = None, scope = None):
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        #函数获取输入最后维度输出通道数,参数min_rank=4限定最少4个维度
        depth_in = slim.utils.last_dimension(inputs.get_shape(), min_rank = 4)
        
        #预处理batch_norm+relu 用于shortcut
...
def resnet_v2(inputs, blocks, num_classes = None, global_pool = True, include_root_block = True, reuse = None, scope = None):
    with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse = reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck, stack_blocks_dense], outputs_collections = end_points_collection):
            net = inputs #(32, 224, 224, 3)

        if include_root_block:
            with slim.arg_scope([slim.conv2d], activation_fn = None, normalizer_fn = None):
                net = conv2d_same(net, 64, 7, stride = 2, scope = 'conv1') #(32, 112, 112, 64)  7*7 64 stride 2 这个卷积没有激活函数和norm函数
            net = slim.max_pool2d(net, [3, 3], stride = 2, scope = 'pool1') #(32, 56, 56, 64)  maxpooling

        #(32, 56, 56, 64)经过残差网络模块，
        ...
        #这里的net为(32, 1, 1, 2048)
        #判断是否要转成num_classes个一维数组，将卷积层最后的结果经过卷积(1*1,1000) (7*7)大小经过该卷积层，变成了
...
#定义50层的ResNet   
#每个如(256, 64, 1)的数据 包含3个卷积层 这里为48层
...
#为slim.conv2d,slim.batch_norm,slim.max_pool2d设置一些默认参数
